...
{
    // Convert tomtom:// to tomtom:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("tomtom://"))
    {
        uri.replace(0, 12, "tomtom:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "tomtom.lnk";
}
...
{
    return GetAutostartDir() / "tomtom.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("tomtom-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  tomtom-qt [" + tr("command-line options") + "]                     " + "\n";
